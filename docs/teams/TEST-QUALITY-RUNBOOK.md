...
61. Resource-effect parser changes need paired evidence: focused parser/security tests that prove non-task `?| resource_operation` forms stay out of task-await ASTs, catch-all fallback parses through the resource-effect route, and named handlers fail closed until their typed dispatch slice lands. Runtime evidence must cover the smallest behavior being changed: discard still commits the underlying topology write, fallback runs only after a materialized resource failure is cleared, successful operations skip fallback, and no-fallback settlement stops before the following statement.
