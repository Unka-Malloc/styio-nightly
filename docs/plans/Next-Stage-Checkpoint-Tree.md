...
- `CP-B0.2` `M18` — **Closed 2026-04-22**
  ...
  - Minimum gate:
    - closed by `ctest --test-dir build-codex -L ide --output-on-failure`, including `StyioLspRuntime.RuntimeDrainCanBeBudgetedForScheduling`, `StyioLspRuntime.IdleSliceDrainsSemanticBeforeBackgroundWork`, and `StyioLspRuntime.RunAdvancesBackgroundWorkAsRequestDrivenFallback`.
