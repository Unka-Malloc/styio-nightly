...
### P0. Resource-effect `?|` is still mostly task-await, not the unified resource effect model
...
1. The parser entry for `?|` now routes through
   ...
   `src/StyioParser/NewParserExpr.cpp`; it preserves typed task-await binding
...
1. Complete the remaining `?| resource_operation` forms in resource-effect
   ...
   task-await compatibility behavior with parser, sema, lowering, runtime, and
   negative tests.
