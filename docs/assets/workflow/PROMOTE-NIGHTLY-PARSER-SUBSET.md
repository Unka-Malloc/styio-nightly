...
```bash
cmake --build build-codex --target styio_security_test styio -j2
ctest --test-dir build-codex -R '^StyioParserEngine\.' --output-on-failure
ctest --test-dir build-codex -R '^parser_shadow_gate_' --output-on-failure
ctest --test-dir build-codex -R '^parser_legacy_entry_audit$' --output-on-failure
git diff --check
```
